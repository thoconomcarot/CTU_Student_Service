...
def is_table_continuation_signal(signal: PageLayoutSignal) -> bool:
    ...
    Vì nhiều bảng dài trong văn bản hành chính bị tách qua nhiều trang, đôi khi chỉ
    trang giữa có đủ header/keyword để được đánh dấu `likely_table=True`. Trang đầu
    hoặc trang cuối của cùng một bảng vẫn có rất nhiều đường kẻ nhưng thiếu keyword,
    nên rule cũ dễ bỏ sót. Hàm này dùng ngưỡng mềm hơn để mở rộng từ trang bảng đã
    phát hiện sang các trang liền kề có dấu hiệu đường kẻ tương tự.
    """

    if signal.likely_table:
        return True

    # Bảng text-PDF thường có nhiều vector line. Với file Nội quy KTX, trang 4 và 6
    # lần lượt có khoảng 32/33 vector line nên cần được xem là bảng tiếp nối.
    if signal.vector_line_count >= 25:
        return True

    # Với PDF scan/render, raster_line_score trung bình cũng là dấu hiệu bảng tiếp nối.
    if signal.raster_line_score >= 0.38:
        return True

    # Có nhiều header bảng nhưng ít đường kẻ vẫn có thể là bảng nhẹ/biểu mẫu.
    if signal.keyword_score >= 2 and (signal.vector_line_count >= 10 or signal.raster_line_score >= 0.25):
        return True
...
def table_pages_from_signals(
    ...
    Mặc định không chỉ lấy trang `likely_table=True`, mà còn mở rộng sang các trang
    liền trước/liền sau nếu chúng có dấu hiệu là phần tiếp nối của bảng. Cách này sửa
    lỗi bảng dài qua nhiều trang, ví dụ bảng Phụ lục bắt đầu ở trang 4, detector chỉ
    bắt chắc trang 5, nhưng trang 4 và 6 vẫn phải đi qua LlamaParse để giữ cột.
    """
    ...
    for page in sorted(direct_pages):
        # Mở rộng lùi và tiến qua các trang liên tiếp có line/table signal mềm.
        for step in (-1, 1):
            cur = page + step
